<div x-data="{ show_dividers: true }" wire:init="setItems">
    <div class="mb-3 pb-5 border-b border-gray-200 sm:flex sm:items-center sm:justify-between">
        <h3 class="text-lg leading-6 font-medium text-gray-900">
            KW {{ str_pad($start_of_week->week, 2, '0', STR_PAD_LEFT) }} {{ $start_of_week->year }} von {{ $start_of_week->format('d.m.Y') }} - {{ $end_of_week->format('d.m.Y') }}
        </h3>
    ...
    <ul class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 mb-3">
        ...
        @empty
           <li class="bg-white rounded-lg shadow px-4 py-2" style="grid-column: 1 / -1;">Keine Ausstrahlungen gefunden.</li>
        @endforelse
    </ul>
</div>
